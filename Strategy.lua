...
function UnBotShowStrategyFrame(name, className)
	...
	local englishClass = UnitClass("target");
	print(englishClass);
	local classIndex = GetClassFlag(string.lower(englishClass));
		
	if (classIndex) == 1 --warrior
		then InitializeStrategyWarrior();
		elseif 
		(classIndex) == 2 --paladin
		then InitializeStrategyPaladin();
		elseif 
		(classIndex) == 3 --rogue
		then InitializeStrategyRogue();
		elseif 
		(classIndex) == 4 --druid
		then InitializeStrategyDruid();
		elseif 
		(classIndex) == 5 --hunter
		then InitializeStrategyHunter();
		elseif 
		(classIndex) == 6 --shaman
		then InitializeStrategyShaman();
		elseif 
		(classIndex) == 7 --mage
		then InitializeStrategyMage();
		elseif 
		(classIndex) == 8 --warlock
		then InitializeStrategyWarlock();
		elseif 
		(classIndex) == 9 --priest
		then InitializeStrategyPriest();
		elseif 
		(classIndex) == 10 --death knight
		then InitializeStrategyDeathKnight();
	end;
...
function StrategyOption_OnClick(btn, strategyFrame, use, name, strategyType, command)
	if (strategyFrame == nil or name == nil or command == nil or command == "") then
		return;
	end

	if (use == false and command == "utgarde keep") then
		command = "wotlk-uk";
	end
	if (use == false and command == "azjol'nerub") then
		command = "wotlk-an";
	end
	if (use == false and command == "nexus") then
		command = "wotlk-nex";
	end
