	...
	if (newFrame.bagsIcon ~= nil) then
		...
		if (itemID ~= nil and itemID > 0) then
			...
			if (needQuery == false) then
				...
				GameTooltip:AddLine("This item has not been cached yet. Click Refresh button to query the server.",1,0,0,1);
			end
			GameTooltip:AddLine(" ",1,1,1,1);
...
local function CreateOptionByParent(fromParent, flushFunc, bagType)
    ...
    -- Filter box Only create if bagType is 0 (use only for Icons Frame display so we can search by part of the filename)
    if bagType == 0 then
        local filterBox = CreateFrame("EditBox", fromParent:GetName() .. "FilterBox", fromParent, "InputBoxTemplate")
        filterBox:SetSize(200, 25)
        filterBox:SetPoint("TOPLEFT", fromParent, "TOPLEFT", 300, 0)
        filterBox:SetAutoFocus(false) -- Prevent auto-focus
        filterBox:SetBackdropColor(0, 0, 0, 0.8)

        -- Event scripts for filtering
        filterBox:SetScript("OnTextChanged", function(self)
            local filterText = self:GetText()
            UnBotFilterIcons(filterText, fromParent) -- Pass both filterText and the frame
        end)
        ...
        filterBox:SetScript("OnEscapePressed", function(self)
            -- local closeFrame = fromParent:GetName() and _G[fromParent:GetName() .. "Close"]
            -- if closeFrame and closeFrame:IsShown() then
                -- closeFrame:Click() -- Close frame properly
            -- else
                -- fromParent:Hide() -- Fallback: hide the frame
            -- end
			self:ClearFocus()
        end)
    end
...
function RecvOnceItemToBags(bagsFrame,info)
	...
	local itemQuality;
	local itemSellPrice;
	name, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, texture, itemSellPrice = GetItemInfo(info);
	...
	local item = {[1] = 0, [2] = itemID, [3] = name, [4] = texture, [5] = needQuery, [6] = itemQuality, [7] = itemCount, [8] = itemSellPrice};
...
function UnBotFilterIcons(filterText, bagsFrame)
    ...
    -- Ensure filterText is valid
    if not filterText or filterText == "" then
        --DEFAULT_CHAT_FRAME:AddMessage("No filter text provided, displaying all icons.")
        
        -- Reset dataGroup to all icons
        bagsFrame.dataGroup = {}
